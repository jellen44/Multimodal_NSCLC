### Principal Component Analysis Script (With no Autoencoders)
...
# The number of features the PCA technique will choose for each modality
n_lncs <- 300
n_mirna <- 300
ngenes <- 500
nprobes <- 500
...
# The number of features PCA will reduce the feature space to
lnc_features <- 30
mirna_features <- 30
meth_features <- 50
gene_features <- 50
...
single_PCA_features <- meth_features+gene_features+mirna_features+lnc_features
...
total_PCA_list <- list()
results.df_singlePCA_list <- list()
...
# 2. PCA Function
PCA_func <- function(train_data, test_data, num_features) {
  
  #PCA Metholodology
  pc <- prcomp(train_data)
  multimodal_pc <- as.data.frame(pc$x[,1:num_features])
  pc_test <- predict(pc, newdata = test_data)
  multimodal_pc_test <- as.data.frame(pc_test[,1:num_features])
  multimodal_pc <- as.data.frame(multimodal_pc)
  multimodal_pc_test <- as.data.frame(multimodal_pc_test)
  
  #Returning List Result
  PCA_list <- list(multimodal_pc, multimodal_pc_test)
  return(PCA_list)
}
...
for (cv in 1:5) {
  ...
  #PCA - first round get data for single multimodal PCA later on (early integration)
  PCA_lnc_list <- PCA_func(LUSC_lncRNA_process, LUSC_lncRNA_process_test, n_lncs)
  LUSC_lncRNA_process_first <- PCA_lnc_list[[1]]
  LUSC_lncRNA_process_test_first <- PCA_lnc_list[[2]]
  
  #Extracting for altogether PCA (rather than all modalities separately)
  lncrna_single_PCA <- LUSC_lncRNA_process_first
  lncrna_single_PCA_test <- LUSC_lncRNA_process_test_first
  
  #Connecting to Barcodes
  lncrna_single_PCA <- as.data.frame(lncrna_single_PCA)
  lncrna_single_PCA$barcode <- lncrna_barcode_train
  lncrna_single_PCA_test <- as.data.frame(cbind(lncrna_single_PCA_test, lncrna_barcode_test))
  colnames(lncrna_single_PCA_test)[ncol(lncrna_single_PCA_test)] <- "barcode"
  
  # Changing Column Names for Single PCA Later On (Early Integration)
  for (i in 1:((ncol(lncrna_single_PCA)-1))) { colnames(lncrna_single_PCA)[i] <- paste0("LNC", i) }
  for (i in 1:((ncol(lncrna_single_PCA_test)-1))) { colnames(lncrna_single_PCA_test)[i] <- paste0("LNC", i) }
  
  #PCA:(reducing LNC directly to 30 features)
  PCA_lnc_list <- PCA_func(LUSC_lncRNA_process, LUSC_lncRNA_process_test, lnc_features)
  LUSC_lncRNA_process <- PCA_lnc_list[[1]]
  LUSC_lncRNA_process_test <- PCA_lnc_list[[2]]
  
  # Changing Column Names for Training and Testing Data
  LUSC_lncRNA_auto <- as.data.frame(LUSC_lncRNA_process)
  for (i in 1:ncol(LUSC_lncRNA_auto)) { colnames(LUSC_lncRNA_auto)[i] <- paste0("V",i) }
  LUSC_lncRNA_auto_output <- as.data.frame(LUSC_lncRNA_process_test)
  for (i in 1:ncol(LUSC_lncRNA_auto_output)) { colnames(LUSC_lncRNA_auto_output)[i] <- paste0("V",i) }
  ...
  #PCA
  PCA_rna_list <- PCA_func(LUSC_RNA_process, LUSC_RNA_process_test,ngenes)
  LUSC_RNA_process_first <- PCA_rna_list[[1]]
  LUSC_RNA_process_tests_first <- PCA_rna_list[[2]]
  
  #Extracting for altogether PCA
  mrna_single_PCA <- LUSC_RNA_process_first
  mrna_single_PCA_test <- LUSC_RNA_process_tests_first
  mrna_single_PCA <- as.data.frame(mrna_single_PCA)
  mrna_single_PCA$barcode <- mrna_barcode_train
  mrna_single_PCA_test <- as.data.frame(cbind(mrna_single_PCA_test, mrna_barcode_test))
  colnames(mrna_single_PCA_test)[ncol(mrna_single_PCA_test)] <- "barcode"
  
  # Changing Column Names for Single PCA Later On (Early Integration)
  for (i in 1:((ncol(mrna_single_PCA)-1))) { colnames(mrna_single_PCA)[i] <- paste0("Gene", i) }
  for (i in 1:((ncol(mrna_single_PCA_test)-1))) { colnames(mrna_single_PCA_test)[i] <- paste0("Gene", i) }
  
  # Reducing mRNA data directly to 50 features
  PCA_rna_list <- PCA_func(LUSC_RNA_process, as.data.frame(LUSC_RNA_process_test), gene_features)
  LUSC_RNA_process <- PCA_rna_list[[1]]
  LUSC_RNA_process_test <- PCA_rna_list[[2]]
  ...
  #PCA
  PCA_mirna_list <- PCA_func(LUSC_miRNA_process, LUSC_miRNA_process_test, n_mirna)
  LUSC_miRNA_process_first <- PCA_mirna_list[[1]]
  LUSC_miRNA_process_test_first <- PCA_mirna_list[[2]]
  
  #Extracting for altogether PCA
  mirna_single_PCA <- LUSC_miRNA_process_first
  mirna_single_PCA_test <- LUSC_miRNA_process_test_first
  mirna_single_PCA <- as.data.frame(mirna_single_PCA)
  mirna_single_PCA$barcode <- miRNA_barcode_train
  mirna_single_PCA_test <- as.data.frame(cbind(mirna_single_PCA_test, miRNA_barcode_test))
  colnames(mirna_single_PCA_test)[ncol(mirna_single_PCA_test)] <- "barcode"
  
  # Changing Column Names for Single PCA Later On (Early Integration)
  for (i in 1:((ncol(mirna_single_PCA)-1))) { colnames(mirna_single_PCA)[i] <- paste0("miRNA", i) }
  for (i in 1:((ncol(mirna_single_PCA_test)-1))) { colnames(mirna_single_PCA_test)[i] <- paste0("miRNA", i) }
  
  #PCA
  PCA_mirna_list <- PCA_func(LUSC_miRNA_process, LUSC_miRNA_process_test, mirna_features)
  LUSC_miRNA_process <- PCA_mirna_list[[1]]
  LUSC_miRNA_process_test <- PCA_mirna_list[[2]]
  ...
  # PCA
  PCA_meth_list <- PCA_func(LUSC_meth_process, LUSC_meth_process_test, nprobes)
  LUSC_meth_process_first <- PCA_meth_list[[1]]
  LUSC_meth_process_test_first <- PCA_meth_list[[2]]
  
  #Extracting for altogether PCA
  meth_single_PCA <- LUSC_meth_process_first
  meth_single_PCA_test <- LUSC_meth_process_test_first
  meth_single_PCA <- as.data.frame(cbind(meth_single_PCA, meth_barcode_train))
  meth_single_PCA_test <- as.data.frame(cbind(meth_single_PCA_test, meth_barcode_test))
  colnames(meth_single_PCA)[ncol(meth_single_PCA)] <- "barcode"
  colnames(meth_single_PCA_test)[ncol(meth_single_PCA_test)] <- "barcode"
  
  # Changing Column Names for Single PCA Later On (Early Integration)
  for (i in 1:((ncol(meth_single_PCA)-1))) { colnames(meth_single_PCA)[i] <- paste0("Meth", i) }
  for (i in 1:((ncol(meth_single_PCA_test)-1))) { colnames(meth_single_PCA_test)[i] <- paste0("Meth", i) }
  
  # PCA - Directly Reducing Data to 50 Features
  PCA_meth_list <- PCA_func(LUSC_meth_process, LUSC_meth_process_test, meth_features)
  LUSC_meth_process <- PCA_meth_list[[1]]
  LUSC_meth_process_test <- PCA_meth_list[[2]]
  ...
  ### Early Integration using PCA Technique
  
  if (one_PCA_early=="yes") {
    #Training Data
    df3 <- merge(lncrna_single_PCA, mirna_single_PCA, by="barcode")
    df3 <- merge(df3, mrna_single_PCA, by="barcode")
    single_PCA <- merge(df3, meth_single_PCA, by="barcode")
    
    #Testing Data
    df4 <- merge(lncrna_single_PCA_test, mirna_single_PCA_test, by="barcode")
    df4 <- merge(df4, mrna_single_PCA_test, by="barcode")
    single_PCA_test <- merge(df4, meth_single_PCA_test, by="barcode")
    
    #Combining with Outcome Data
    outcome_data_PCA <- LUSCclin_lung %>% dplyr::select(c(barcode, vital_status, censor_time1))
    single_PCA <- merge(single_PCA, outcome_data_PCA, by="barcode")
    single_PCA_test <- merge(single_PCA_test, outcome_data_PCA, by="barcode")
    
    #Save vital status and outcome into a dataframe
    single_PCA_outcome <- single_PCA %>% dplyr::select(c(barcode, vital_status, censor_time1))
    single_PCA_outcome_test <- single_PCA_test %>% dplyr::select(c(barcode, vital_status, censor_time1))
    
    #Correct Indices Train
    full_LUSC_indices <- merge(single_PCA, barcode_disease_mapping, by="barcode")
    ...
    full_LUSC_indices_test <- merge(single_PCA_test, barcode_disease_mapping, by="barcode")
    ...
    #Removing Barcodes
    single_PCA <- single_PCA %>% dplyr::select(-c(barcode, vital_status, censor_time1))
    single_PCA_test <- single_PCA_test %>% dplyr::select(-c(barcode, vital_status, censor_time1))
    single_PCA <- as.data.frame(sapply(single_PCA, as.numeric))
    single_PCA_test <- as.data.frame(sapply(single_PCA_test, as.numeric))
    
    #PCA
    PCA_list <- PCA_func(single_PCA, single_PCA_test, single_PCA_features)
    single_PCA <- PCA_list[[1]]
    single_PCAtest <- PCA_list[[2]]
    
    #incorporating outcome data
    LUSC_PCA <- as.data.frame(cbind(single_PCA, single_PCA_outcome))
    LUSC_PCA_test <- as.data.frame(cbind(single_PCAtest,single_PCA_outcome_test))
    
    #incorporating clinical data
    LUSC_PCA <- LUSC_PCA %>% dplyr::select(-c(vital_status, censor_time1))
    LUSC_PCA_test <- LUSC_PCA_test %>% dplyr::select(-c(vital_status, censor_time1))
    LUSC_single_PCA <- merge(LUSC_PCA, clin_train, by="barcode")
    LUSC_single_PCA_test <- merge(LUSC_PCA_test, clin_test, by="barcode")
    
    #Making Numeric
    LUSC_single_PCA <- LUSC_single_PCA %>% dplyr::select(-c(barcode))
    LUSC_single_PCA_test <- LUSC_single_PCA_test %>% dplyr::select(-c(barcode))
    LUSC_single_PCA <- as.data.frame(sapply(LUSC_single_PCA, as.numeric))
    LUSC_single_PCA_test <- as.data.frame(sapply(LUSC_single_PCA_test, as.numeric))
    
    #Elastic Net
    multimodal_test_LUSC <- LUSC_single_PCA_test[c(full_LUSC_indices_test), ]
    multimodal_test_LUAD <- LUSC_single_PCA_test[c(full_LUAD_indices_test), ]
    elasticnet_cindex_list <- elastic_net_func(LUSC_single_PCA, LUSC_single_PCA_test, multimodal_test_LUSC, multimodal_test_LUAD)
    Cindex_Test <- elasticnet_cindex_list[[1]]
    Cindex_LUSC <- elasticnet_cindex_list[[2]]
    Cindex_LUAD <- elasticnet_cindex_list[[3]]
  ...
  if (one_PCA_early=="yes") { df1 <- rbind(df1,results.df_fullPCA) }
