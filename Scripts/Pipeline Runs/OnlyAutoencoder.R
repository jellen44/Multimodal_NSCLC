# Autoencoder Only Script (No Linear Feature Selection)
...
for (u in 1:length(activation_function_set)) {
  for (z in 1:length(zeros_percentage_set)) {
    ...
    label <- paste0(activation_function, zeros_percentage, "Zeros_OnlyAE_")
    ...
    for (cv in 1:5) {
      ...
      # Autoencoder To Get Feature Space for Early Integration Later On
      
      ### Denoising Zeros Autoencoder
      if (autoencoder_type == "denoising_zeros") { 
        denoising_zeros_list <- denoising_zeros_func(LUSC_lncRNA_process, LUSC_lncRNA_process_test, n_lncs)
        LUSC_lncRNA_first <- denoising_zeros_list[[1]]
        LUSC_lncRNA_first_output <- denoising_zeros_list[[2]] }
      
      ### Denoising (Gaussian) Autoencoder
      if (autoencoder_type=="denoising_gaussian") {
        denoising_gaussian_list <- denoising_gaussian_func(LUSC_lncRNA_process, LUSC_lncRNA_process_test, n_lncs) 
        LUSC_lncRNA_first <- denoising_gaussian_list[[1]]
        LUSC_lncRNA_first_output <- denoising_gaussian_list[[2]]  }
      
      #Extracting for altogether AutoEncoder (rather than all modalities separately)
      lncrna_fullAE <- LUSC_lncRNA_first
      lncrna_fullAE_test <- LUSC_lncRNA_first_output 
      
      #Connecting to Barcodes
      lncrna_fullAE <- as.data.frame(lncrna_fullAE)
      ...
      # Changing Column Names for Single AE Later On (Early Integration)
      for (i in 1:((ncol(lncrna_fullAE)-1))) { colnames(lncrna_fullAE)[i] <- paste0("LNC", i) }
      for (i in 1:((ncol(lncrna_fullAE_test)-1))) { colnames(lncrna_fullAE_test)[i] <- paste0("LNC", i) }
      ...
      ### Denoising Zeros Autoencoder
      if (autoencoder_type == "denoising_zeros") { 
        denoising_zeros_list <- denoising_zeros_func(LUSC_RNA_process, LUSC_RNA_process_test, ngenes)
        LUSC_RNA_first <- denoising_zeros_list[[1]]
        LUSC_RNA_first_output <- denoising_zeros_list[[2]] }
      
      ### Denoising (Gaussian) Autoencoder
      if (autoencoder_type=="denoising_gaussian") {
        denoising_gaussian_list <- denoising_gaussian_func(LUSC_RNA_process, LUSC_RNA_process_test, ngenes) 
        LUSC_RNA_first <- denoising_gaussian_list[[1]]
        LUSC_RNA_first_output <- denoising_gaussian_list[[2]]  }
      
      #Extracting for altogether AE (early integration)
      mrna_fullAE <- LUSC_RNA_first
      mrna_fullAE_test <- LUSC_RNA_first_output
      mrna_fullAE <- as.data.frame(mrna_fullAE)
      ...
      # Changing Column Names for Single AE Later On (Early Integration)
      for (i in 1:((ncol(mrna_fullAE)-1))) { colnames(mrna_fullAE)[i] <- paste0("Gene", i) }
      for (i in 1:((ncol(mrna_fullAE_test)-1))) { colnames(mrna_fullAE_test)[i] <- paste0("Gene", i) }
      ...
      ### Denoising Zeros Autoencoder
      if (autoencoder_type == "denoising_zeros") { 
        denoising_zeros_list <- denoising_zeros_func(LUSC_miRNA_process, LUSC_miRNA_process_test, n_mirna)
        LUSC_mirna_first <- denoising_zeros_list[[1]]
        LUSC_mirna_first_output <- denoising_zeros_list[[2]] }
      
      ### Denoising (Gaussian) Autoencoder
      if (autoencoder_type=="denoising_gaussian") {
        denoising_gaussian_list <- denoising_gaussian_func(LUSC_miRNA_process, LUSC_miRNA_process_test, n_mirna) 
        LUSC_mirna_first <- denoising_gaussian_list[[1]]
        LUSC_mirna_first_output <- denoising_gaussian_list[[2]]  }
      
      #Extracting for altogether AE
      mirna_fullAE <- LUSC_mirna_first
      mirna_fullAE_test <- LUSC_mirna_first_output
      mirna_fullAE <- as.data.frame(mirna_fullAE)
      ...
      # Changing Column Names for Single AE Later On (Early Integration)
      for (i in 1:((ncol(mirna_fullAE)-1))) { colnames(mirna_fullAE)[i] <- paste0("miRNA", i) }
      for (i in 1:((ncol(mirna_fullAE_test)-1))) { colnames(mirna_fullAE_test)[i] <- paste0("miRNA", i) }
      ...
      ### Denoising Zeros Autoencoder
      if (autoencoder_type == "denoising_zeros") { 
        denoising_zeros_list <- denoising_zeros_func(LUSC_meth_process, LUSC_meth_process_test, nprobes)
        LUSC_meth_first <- denoising_zeros_list[[1]]
        LUSC_meth_first_output <- denoising_zeros_list[[2]] }
      
      ### Denoising (Gaussian) Autoencoder
      if (autoencoder_type=="denoising_gaussian") {
        denoising_gaussian_list <- denoising_gaussian_func(LUSC_meth_process, LUSC_meth_process_test, nprobes) 
        LUSC_meth_first <- denoising_gaussian_list[[1]]
        LUSC_meth_first_output <- denoising_gaussian_list[[2]]  }
      
      #Extracting for altogether AE
      meth_fullAE <- LUSC_meth_first
      meth_fullAE_test <- LUSC_meth_first_output
      ...
      # Changing Column Names for Single AE Later On (Early Integration)
      for (i in 1:((ncol(meth_fullAE)-1))) { colnames(meth_fullAE)[i] <- paste0("Meth", i) }
      for (i in 1:((ncol(meth_fullAE_test)-1))) { colnames(meth_fullAE_test)[i] <- paste0("Meth", i) }
      ...
      ## Early Integration using one autoencoder
      ...
      if (one_AE_early=="yes") {
        ...
        outcome_data_fullAE <- LUSCclin_lung %>% dplyr::select(c(barcode, vital_status, censor_time1))
        fullAE <- merge(fullAE, outcome_data_fullAE, by="barcode")
        fullAE_test <- merge(fullAE_test, outcome_data_fullAE, by="barcode")
        
        #Save vital status and outcome into a dataframe
        fullAE_outcome <- fullAE %>% dplyr::select(c(barcode, vital_status, censor_time1))
        fullAE_outcome_test <- fullAE_test %>% dplyr::select(c(barcode, vital_status, censor_time1))
        ...
        #Removing Barcodes
        fullAE <- fullAE %>% dplyr::select(-c(barcode, vital_status, censor_time1))
        fullAE_test <- fullAE_test %>% dplyr::select(-c(barcode, vital_status, censor_time1))
        fullAE <- as.data.frame(sapply(fullAE, as.numeric))
        fullAE_test <- as.data.frame(sapply(fullAE_test, as.numeric))
            
        ### Denoising Zeros Autoencoder
        if (autoencoder_type == "denoising_zeros") { 
          denoising_zeros_func(fullAE, fullAE_test, fullAE_features) 
        ...
        ## Other Autoencoder Type: Denoising Gaussian
        if (autoencoder_type=="denoising_gaussian") {
          denoising_gaussian_list <- denoising_gaussian_func(fullAE, fullAE_test, fullAE_features) 
        ...
        LUSC_AE <- LUSC_AE %>% dplyr::select(-c(vital_status, censor_time1))
        LUSC_AE_test <- LUSC_AE_test %>% dplyr::select(-c(vital_status, censor_time1))
      ...
      if (one_AE_early=="yes") { df1 <- rbind(df1, results.df_fullAE) }
