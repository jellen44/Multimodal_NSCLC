...
if (!require(lumi)) install.packages('lumi')
if (!require(statmod)) install.packages('statmod')
if (!require(IlluminaHumanMethylation450kanno.ilmn12.hg19)) install.packages('IlluminaHumanMethylation450kanno.ilmn12.hg19')
```
...
load(file="~/desktop/Multimodal/DEA/DEA_Files/Methylation_Processed_Imputed.Rdata") # This file is preprocessed methylation data, but taken right after the missing data imputation step in the 'All_Modality_Comparison.R' script

# Setting up Methylation Data
LUAD_meth <- as.data.frame(t(LUSC_meth_imputed))
...
# Loop that makes sure no infinite values were created
infinite_values <- NULL
for (i in 1:(ncol(data_new)-1)) {
    if (any(is.infinite(data_new[,i]))) {
         infinite_values[i]  <- i 
         } else infinite_values[i]  <- NA
}
infinite_indices <- na.omit(infinite_values)
data_new <- data_new[,-infinite_indices]
